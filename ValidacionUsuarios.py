...
if usuario['categoria_usuario_id']['name'] != 'Administracion':
    for obj in self:
        ...
        # Sale de Pendiente
        if obj.stage_id.codigo == 'PROG' and obj.categoria_id.x_aplica_programacion:
            
            if obj.categoria_id.codigo in categorias_alcanzadas_por_deposito:
                #si es civil cota 0 y serv. tecnico saltea validacion
                if(not(obj.categoria_id.codigo == 'v202204_IEC0' and obj.area_id.codigo == 'SETE')):
                    if not obj.x_x_materiales_ids:
